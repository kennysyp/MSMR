...
def _read_gene(omics_dir, split):
    ...
    return df
# def _read_histo(split_dir, split, fold_idx, dataset_name):
#     base_path = j_(split_dir, f'{dataset_name}_overall_survival_k={fold_idx}')
#     print(f"Reading {split}.csv from {base_path}")
#     split_path = j_(base_path, f'{split}.csv')
...
# def _read_gene(omics_dir, omics_dataset_name):
#     split_path = j_(omics_dir, omics_dataset_name, 'rna_clean.csv')
#     if os.path.isfile(split_path):
#         df = pd.read_csv(split_path, engine='python', index_col=0)
#         assert 'Unnamed: 0' not in df.columns
...
# def read_splits(args, num_folds):
#     splits_csvs = {}
#     split_names = args.split_names.split(',')
#     print(f"Using the following split names: {split_names}")
...
#         for split in split_names:
#             splits_csvs[(split, fold_idx)] = {}
#             splits_csvs[(split, fold_idx)]['histo'] = _read_histo(args.split_dir, split, fold_idx, args.dataset_name)

#             if 'omics_dir' in args and 'omics_dataset_name' in args:
#                 splits_csvs[(split, fold_idx)]['gene'] = _read_gene(args.omics_dir, args.omics_dataset_name)
#             else:
#                 splits_csvs[(split, fold_idx)]['gene'] = None
