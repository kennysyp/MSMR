...
def mog_eval(mog, data):
    ...
    # print("1./Sigma.permute(0,2,1) shape:", (1./Sigma.permute(0,2,1)).shape)
...
class DirNIWNet(nn.Module):
    ...
    def map_m_step(self, data, weight, tau=1.0, prior=None):
        ...
        # mu = (1 - alpha) * mu + alpha * ((wxsum + m.unsqueeze(0) * tau) / wsum_reg.unsqueeze(-1))   
        Sigma = (wxxsum + (V+m**2).unsqueeze(0)*tau) / wsum_reg.unsqueeze(-1) - mu**2   
        # Sigma = (wxxsum + (V + m**2).unsqueeze(0) * tau) / wsum_reg.unsqueeze(-1) - mu**2 + reg_term  
