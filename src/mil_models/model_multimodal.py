...
from .components import SNN_Block, MMAttentionLayer, FeedForward, FeedForwardEnsemble, process_surv
from .components import SNN_Residual_Block, SNN_Residual_Block_2, SNN_Residual_Block_3
# from .components import Attn_Net_Gated

from mil_models.kan import KAN
...
def init_per_path_model(omic_sizes, hidden_dim=256):
    ...
    for input_dim in omic_sizes:
        ...
        for i, _ in enumerate(hidden[1:]):
            fc_omic.append(SNN_Block(dim1=hidden[i], dim2=hidden[i + 1], dropout=0.25)) # (dim1, dim2)——(256, 256)
...
def construct_proto_embedding(path_proj_dim, append_embed='modality', numOfproto_histo=16, numOfproto_omics=50):
    ...
    print(f"histo_embedding shape: {histo_embedding.shape if histo_embedding is not None else 'None'}")
    print(f"gene_embedding shape: {gene_embedding.shape if gene_embedding is not None else 'None'}")
...
class coattn(nn.Module):
    def __init__(
        ...
        if self.histo_model == 'panther':  # Uses prob/mean/cov  
            ...
            self.path_proj_net = nn.Sequential(nn.Linear(self.histo_in_dim + 1, path_proj_dim))  
        ...
        self.classifier = KAN([in_dim, 140, self.num_classes])   
    ...
    def forward_no_loss(self, x_path, x_omics, return_attn=False):
        ...
        if self.histo_model == 'mil':
            ...
            wsi_postSA_embed = agg_histo(wsi_postSA_embed, self.histo_agg)  # 'mean/cat'
