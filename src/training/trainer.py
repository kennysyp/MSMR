...
def train(datasets, args):
    ...
    if args.model_histo_type in PROTO_MODELS:
        ...
        for k, loader in datasets.items():
            ...
            tensors_to_cat = [torch.Tensor(prob).unsqueeze(dim=-1), torch.Tensor(mean)]
            if cov is not None:
                tensors_to_cat.append(torch.Tensor(cov))
            loader.dataset.X = torch.cat(tensors_to_cat, dim=-1)
            factor = args.n_proto 
...
def validate_survival(model, loader,
    ...
    if model.__class__.__name__ == 'coattn':
        all_omic_attn, all_cross_attn, all_path_attn = [], [], []
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        if model.__class__.__name__ == 'coattn' and return_attn:
